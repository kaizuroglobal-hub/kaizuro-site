...
(function () {
  ...
  function plainHtml(value){var d=document.createElement("div");d.innerHTML=value||"";return d.textContent.trim();}
  ...
  function applyEngineering(d){if(!d)return;var s=q("#details");txt(".eyebrow",d.section_eyebrow,s);lines("#guide-title span",[d.section_title_line1,d.section_title_line2],s);html(".story-copy>p:not(.eyebrow)",d.section_body,s);txt(".story-copy>strong",d.section_tagline,s);attr("img","src",d.section_image_src,s);attr("img","alt",d.section_image_alt,s);}
  function applyScrollingCards(d){if(!d||!Array.isArray(d.cards))return;window.kzChapterData=d.cards;qa("[data-chapter]").forEach(function(f){var x=d.cards[Number(f.dataset.chapter)];if(!x)return;txt(".mobile-chapter-copy span",x.eyebrow,f);txt(".mobile-chapter-copy b",x.title,f);html(".mobile-chapter-copy p",x.mobile_text,f);attr("img","src",x.image_src,f);attr("img","alt",x.image_alt,f);});var x=d.cards[0];if(x){txt("[data-chapter-eyebrow]",x.eyebrow);txt("[data-chapter-title]",x.title);html("[data-chapter-text]",x.text);txt("[data-chapter-index]","01 / "+String(d.cards.length).padStart(2,"0"));}}
  function applyGrip(d){if(!d)return;var s=q("#grip");txt(".eyebrow",d.eyebrow,s);txt("#grip-title",d.title,s);html(".story-copy>p:not(.eyebrow)",d.body,s);qa(".detail-list span",s).forEach(function(el,i){var x=d.features&&d.features[i];if(!x)return;var b=q("b",el);if(b)b.textContent=x.title;Array.from(el.childNodes).filter(function(n){return n!==b;}).forEach(function(n){n.remove();});el.appendChild(document.createTextNode(" "+plainHtml(x.body)));});attr("img","src",d.image_src,s);attr("img","alt",d.image_alt,s);}
  ...
  function applyEvolution(d){if(!d)return;var s=q("#evolution");txt(".roadmap-copy .eyebrow",d.eyebrow,s);txt("#evolution-title",d.title,s);html(".roadmap-copy>p:not(.eyebrow)",d.body,s);var list=q(".prototype-timeline",s);if(list&&Array.isArray(d.steps)){list.innerHTML="";d.steps.forEach(function(x){var li=document.createElement("li"),n=document.createElement("span"),st=document.createElement("strong"),p=document.createElement("p");n.textContent=x.number||"";st.textContent=x.title||"";p.innerHTML=x.body||"";if(p.children.length===1&&p.firstElementChild.tagName==="P")p.innerHTML=p.firstElementChild.innerHTML;li.append(n,st,p);list.appendChild(li);});}txt(".evolution-close",d.closing_line,s);}
  ...
  function applyFounderDelivery(d){if(!d)return;var s=q(".founder-roadmap");txt(".eyebrow",d.eyebrow,s);txt("#delivery-title",d.title,s);html(":scope>div>p:not(.eyebrow)",d.body,s);var list=q(".production-roadmap",s);if(list&&Array.isArray(d.milestones)){list.innerHTML="";d.milestones.forEach(function(x){var li=document.createElement("li"),n=document.createElement("span"),st=document.createElement("strong"),p=document.createElement("p");n.textContent=x.number||"";st.textContent=x.title||"";p.innerHTML=x.body||"";if(p.children.length===1&&p.firstElementChild.tagName==="P")p.innerHTML=p.firstElementChild.innerHTML;li.append(n,st,p);list.appendChild(li);});}}
  function applyTerms(d){if(!d)return;var s=q("#terms");txt(".eyebrow",d.eyebrow,s);txt("#terms-title",d.title,s);var list=q(".terms-list",s);if(list&&Array.isArray(d.items)){list.innerHTML="";d.items.forEach(function(x){var details=document.createElement("details"),summary=document.createElement("summary"),p=document.createElement("p");if(x.open)details.open=true;summary.textContent=x.question||"";p.innerHTML=x.answer||"";if(p.children.length===1&&p.firstElementChild.tagName==="P")p.innerHTML=p.firstElementChild.innerHTML;details.append(summary,p);list.appendChild(details);});}}
  ...
  function applyAdvanced(d) {
    ...
    if (Array.isArray(d.css_variables)) d.css_variables.forEach(function (item) { if (item && item.name) document.documentElement.style.setProperty(item.name, item.value == null ? "" : String(item.value)); });
    if (d.custom_css) { var style = document.getElementById("kaizuro-cms-custom-css") || document.createElement("style"); style.id = "kaizuro-cms-custom-css"; style.textContent = String(d.custom_css); if (!style.parentNode) document.head.appendChild(style); }
    if (Array.isArray(d.element_rules)) d.element_rules.forEach(function (rule) {
      if (!rule || rule.enabled === false || !rule.selector || !rule.action) return;
      qa(rule.selector).forEach(function (el) {
        var value = rule.value == null ? "" : String(rule.value), key = rule.attribute == null ? "" : String(rule.attribute);
        switch (rule.action) {
          ...
          case "visibility": if (value === "hidden" || value === "false" || value === "0") { el.hidden = true; el.style.display = "none"; } else { el.hidden = false; el.style.removeProperty("display"); } break;
          case "remove": el.remove(); break;
        }
      });
    });
    if (d.custom_javascript) try { Function("window", "document", "settings", '"use strict";\n' + String(d.custom_javascript))(window, document, window.KAIZURO_CMS_SETTINGS); } catch (error) { console.error("[CMS] Custom JavaScript failed", error); }
  }

  var handlers={site:applySite,hero:applyHero,story:applyStory,assault:applyAssault,principles:applyPrinciples,engineering:applyEngineering,scrolling_cards:applyScrollingCards,grip:applyGrip,reel:applyReel,proof:applyProof,specifications:applySpecifications,evolution:applyEvolution,founder:applyFounder,founder_delivery:applyFounderDelivery,terms:applyTerms,halo:applyHalo,footer:applyFooter};
