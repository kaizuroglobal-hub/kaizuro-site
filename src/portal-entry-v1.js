...
async function login(request,env){const origin=request.headers.get("Origin");if(origin&&origin!==new URL(request.url).origin)return html(page("Invalid sign-in request."),403);const f=await request.formData();const email=String(f.get("email")||"").trim().toLowerCase();const password=String(f.get("password")||"");if(email!==ADMIN_EMAIL||await sha(password)!==ADMIN_PASSWORD_SHA)return html(page("Email or password is incorrect."),401);const exp=Date.now()+8*60*60*1000;const body=`${ADMIN_EMAIL}|${exp}`;const value=encodeURIComponent(`${body}|${await sign(body,env)}`);return new Response(null,{status:303,headers:{"Location":ROOT,"Set-Cookie":`kz_admin=${value}; Path=${ROOT}; HttpOnly; Secure; SameSite=Strict; Max-Age=28800`,`Cache-Control":"no-store"}})}
...
export default {async fetch(request,env,ctx){const url=new URL(request.url);const host=url.hostname.toLowerCase();const path=url.pathname.replace(/\/$/,"")||"/";
  ...
  if(host===HOST&&(path===ROOT||path.startsWith(`${ROOT}/`))){const {default:admin}=await import("./kaizuro-admin.js");return admin.fetch(request,env,ctx)}
