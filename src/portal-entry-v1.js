...
export default {async fetch(request,env,ctx){const url=new URL(request.url);const host=url.hostname.toLowerCase();const path=url.pathname.replace(/\/$/,"")||"/";
  ...
  if(host===HOST&&(path===ROOT||path.startsWith(`${ROOT}/`))){
    if(!(await hasValidAdminCookie(request,env)))return Response.redirect(new URL(LOGIN,request.url).toString(),302);
    const {default:admin}=await import("./kaizuro-admin.js");
