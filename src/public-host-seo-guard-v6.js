...
function loginPage(message="") {
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow,noarchive"><title>KAIZURO | Admin</title><link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">${LOGIN_CSS}</head><body><main class="login"><div class="brand">KAIZURO</div><p class="eyebrow">Internal Admin</p><h1>Network control.</h1><p class="copy">Restricted KAIZURO access. Dealer credentials cannot access this area.</p>${message?`<div class="notice">${esc(message)}</div>`:""}<form method="post" action="${LOGIN_PATH}"><label>Email<input type="email" name="email" autocomplete="username" required></label><label>Password<input type="password" name="password" autocomplete="current-password" required></label><button type="submit">Sign in</button></form></main></body></html>`;
}
...
async function portalLogin(request, env) {
  if (!originAllowed(request)) return htmlResponse(loginPage("Invalid sign-in request."),403);
